...
async function consultarCPF(cpf) {
  try {
    ...
    const response = await fetch(`https://api.arkz.tech/api/cpf/paullylindo/${cpf}`);
    ...
    nomeMae.textContent = result.mae?.trim() || 'Não informado';
    cpfUsuario.textContent = result.cpfformatado || result.cpf || 'Não informado';
    nomeUsuario.textContent = result.nome || 'Não informado';
    sexoUsuario.textContent = 'Não informado';
    dataNascimento.textContent = result.nascimento || 'Não informado';

    const dadosUsuario = {
      cpf: result.cpf || '',
      nome: result.nome || '',
      sexo: '',
      nomeMae: result.mae?.trim() || '',
  ...
  function redirecionarParaChat() {
    ...
    try {
      ...
      window.location.href = '../../inicio/02' + window.location.search;
    } catch (error) {
